...
# This method must be placed outside of class IndexViewTests
...
class PageMethodTests(TestCase):
    def test_ensure_views_are_positive(self):
        page = Page(category=Category(name='test'), title="test page 1", url="http://www.example.com", views=-1)
        page.save()
        self.assertEqual((page.views >= 0), False)

    def test_ensure_first_visit_not_in_the_future(self):
        future_time = timezone.now() + datetime.timedelta(days=1)
        page = Page(category=Category(name='test'), title="test page 2", url="http://www.example.com", first_visit=future_time)
        page.save()
        self.assertEqual((page.first_visit <= timezone.now()), False)
    def test_ensure_last_visit_not_in_the_future(self):
        future_time = timezone.now() + datetime.timedelta(days=1)
        page = Page(category=Category(name='test'), title="test page 3", url="http://www.example.com", last_visit=future_time)
        page.save()
    ...
    def test_ensure_last_visit_later_than_first_visit(self):
        ...
        now = timezone.now()
        page = Page(category=Category(name='test'), title="test page 4", url="http://www.example.com", first_visit=now, last_visit=past_time) # Here first_visit and last_visit can't be assigned as timezone.now, which is an instance of now()
        page.save()
